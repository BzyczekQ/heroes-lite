...
function generateMap(r, W, H, n) {
  ...
  for (let k = 0; k < scatter; k++) {
    ...
    else if (roll < 0.84) { add({ type: 'observatory', x, y, guard: neutralGuard(r, ri(r, 1, 2)) }); }
    else if (roll < 0.90) { add({ type: 'shrine', x, y, guard: neutralGuard(r, ri(r, 1, 2)) }); }
    else if (roll < 0.95) { add({ type: 'market', x, y }); }
    else { const str = ri(r, 1, 3); const nk = NEUTRALS[ri(r, 0, NEUTRALS.length - 1)].key; add({ type: 'dwelling', x, y, unitKey: nk, guard: neutralGuard(r, str), stored: 0, owner: -1 }); }
  }
...
function reachable(state, sx, sy, mp) {
  ...
  while (open.length) {
    ...
    // ruch po ukosie (koszt ×√2 ≈ ×1.41, jak w HoMM3)
    for (const [dx, dy] of diag) {
      const nx = cur.x + dx, ny = cur.y + dy;
      if (tileBlockedFor(state, nx, ny)) continue;
      // nie przecinaj rogów przez zablokowane kafelki
      if (tileBlockedFor(state, cur.x + dx, cur.y) && tileBlockedFor(state, cur.x, cur.y + dy)) continue;
      const step = Math.round(TERRAIN[state.map.tiles[ny][nx]].cost * 1.41);
      const nd = cur.d + step;
      if (nd > mp) continue;
      const nk = nx + ',' + ny;
      if (dist[nk] === undefined || nd < dist[nk]) { dist[nk] = nd; prev[nk] = ck; open.push({ x: nx, y: ny, d: nd }); }
...
function execMelee(state, atkId, defId) {
  ...
  c.lastAnim = { type: 'melee', atkId, defId, dmg, killed, atkX0, atkY0, t: Date.now() };
  if (def.alive && def.count > 0 && adjacentHex(atk, def) && !def.retaliated) {
    ...
    c.lastAnim.retal = { dmg: rDmg, killed: b2 - atk.count };
  }
...
function execShoot(state, atkId, defId) {
  ...
  c.lastAnim = { type: 'shoot', atkId, defId, dmg, killed: before - def.count, atkX0: atk.x, atkY0: atk.y, t: Date.now() };
  afterCombatAction(state);
}
...
function applyCombatResult(state) {
  ...
  // flagowane siedlisko (po pokonaniu strażnika)
  if (def.flagDwellingId && res.attackerWon) {
    const d = state.map.objects[def.flagDwellingId];
    if (d) { d.owner = atk.player; d.stored = d.stored || 0; delete d.guard; }
  }
  // obserwatorium (odkryj obszar po pokonaniu strażnika)
  if (def.revealId && res.attackerWon) {
    const ob = state.map.objects[def.revealId];
    if (ob) {
      const f = state.fog[atk.player];
      if (f) for (let dy = -6; dy <= 6; dy++) for (let dx = -6; dx <= 6; dx++) {
        const x = ob.x + dx, y = ob.y + dy;
        if (inB(x, y, state.W, state.H) && dx * dx + dy * dy <= 36) { f.exp[y][x] = true; }
      }
    }
  }
  // miasto
...
function startPlayerDay(state, pi) {
  ...
  state.month = Math.floor((state.day - 1) / 28) + 1;
  state.dayEvent = { day: state.day, week: state.week, month: state.month,
    newWeek: state.day > 1 && (state.day - 1) % 7 === 0,
    newMonth: state.day > 1 && (state.day - 1) % 28 === 0 };
...
function buildInTown(state, townId, what) {
  ...
  } else if (what === 'mageGuild') {
    if (t.mageGuild) return false;
    cost = { gold: 2000, wood: 5, mercury: 5 };
  ...
  else if (what === 'mageGuild') t.mageGuild = true;
...
// Nauka czarów w gildii magów
function learnSpells(state, hero, townId) {
  const t = townById(state, townId);
  if (!t || !t.mageGuild) return [];
  if (!hero.spells) hero.spells = [];
  const learned = [];
  for (const key of Object.keys(D.SPELLS)) {
    if (!hero.spells.includes(key)) { hero.spells.push(key); learned.push(key); }
  }
  hero.spellbook = true;
  return learned;
}
...
/* ============================================================
   NOWE: ZATRUDNIANIE BOHATERA, TARG, OBIEKTY, ZAPIS
   ============================================================ */
const HERO_HIRE_COST = 2500;

function hireHero(state, townId) {
  const t = townById(state, townId);
  if (!t) return null;
  const p = state.players[t.owner];
  if (p.heroes.length >= 8) return null;                  // limit 8 bohaterów
  if (!canAfford(p, { gold: HERO_HIRE_COST })) return null;
  // znajdź wolne pole przy mieście
  let spot = { x: t.x, y: t.y };
  for (let r = 1; r <= 2 && spot.x === t.x && spot.y === t.y; r++) {
    for (let dy = -r; dy <= r; dy++) for (let dx = -r; dx <= r; dx++) {
      const x = t.x + dx, y = t.y + dy;
      if (inB(x, y, state.W, state.H) && !tileBlockedFor(state, x, y) && !heroAt(state, x, y)) { spot = { x, y }; }
    }
  }
  payCost(p, { gold: HERO_HIRE_COST });
  const id = 'h' + Date.now() + '' + Math.floor(Math.random() * 999);
  const r = mulberry32(state.seed ^ p.heroes.length * 7919);
  const hero = {
    id, name: heroName(p.heroes.length, r), x: spot.x, y: spot.y, mp: D.HERO_BASE_MP,
    atk: 1 + ri(r, 0, 2), def: 1 + ri(r, 0, 2), level: 1, xp: 0, mana: 10, maxMana: 20,
    army: [{ key: factionUnit(p.faction, 1).key, count: 5 }],
    faction: p.faction, alive: true, hasMoved: false
  };
  p.heroes.push(hero);
  recomputeVision(state, p.idx);
  return hero;
...
// TARG: wymiana surowców. Kurs zależy od liczby targów gracza (proste).
function tradeResource(state, pi, fromRes, toRes, amount) {
  const p = state.players[pi];
  amount = Math.min(amount, p.resources[fromRes] || 0);
  if (amount <= 0 || fromRes === toRes) return 0;
  // kurs (multiplikator ile zyskujesz za 1 oddany): złoto drogie
  let mult;
  if (fromRes === 'gold') mult = 1 / 5;        // 5 złota = 1 surowiec
  else if (toRes === 'gold') mult = 1 / 2;     // 2 surowce = 1 złoto
  else mult = 1 / 2;                            // 2 surowce = 1 inny surowiec
  const gained = Math.floor(amount * mult);
  p.resources[fromRes] -= amount;
  p.resources[toRes] = (p.resources[toRes] || 0) + gained;
  return gained;
}

// Odkrycie obszaru mgły (obserwatorium / wieża)
function revealArea(state, pi, cx, cy, radius) {
  const f = state.fog[pi];
  if (!f) return;
  for (let dy = -radius; dy <= radius; dy++) for (let dx = -radius; dx <= radius; dx++) {
    const x = cx + dx, y = cy + dy;
    if (!inB(x, y, state.W, state.H)) continue;
    if (dx * dx + dy * dy <= radius * radius) { f.exp[y][x] = true; f.vis[y][x] = true; }
  }
}

// Dzielenie stosu jednostek
function splitStack(army, idx, count, intoNewSlot) {
  if (!army[idx] || count <= 0 || count >= army[idx].count) return false;
  army[idx].count -= count;
  if (intoNewSlot) army.push({ key: army[idx].key, count });
  return true;
}

// Świątynia / figura: daje XP i doświadczenie
function grantXP(state, hero, amount) { addXP(hero, amount); }

// Flagowane siedlisko na mapie — zbierz jednostki
function collectDwelling(state, hero, objId) {
  const o = state.map.objects[objId];
  if (!o || o.type !== 'dwelling' || o.owner !== heroPlayer(state, hero).idx) return 0;
  const n = o.stored || 0;
  if (n <= 0) return 0;
  addStack(hero.army, o.unitKey, n);
  o.stored = 0;
  return n;
}

// ZAPIS / WCZYTANIE (localStorage w przeglądarce)
function serialize(state) {
  // zamień tablice mgły na kompaktową formę (nie ma potrzeby, JSON sobie poradzi)
  return JSON.stringify(state);
}
function deserialize(str) {
  const state = JSON.parse(str);
  // upewnij się, że fog istnieje
  if (!state.fog) initFog(state);
  return state;
}
...
const Engine = {
  ...
  hireHero, tradeResource, revealArea, splitStack, collectDwelling,
  serialize, deserialize, heroPlayer, grantXP, learnSpells,
  D
};
